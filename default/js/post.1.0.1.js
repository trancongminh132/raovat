...
var post = 
{
	...
	restorePost : function(Id)
	{
		$(this).myBoxy (Boxy,{
			type: 'confirm',
			message: 'Bạn có muốn khôi phục lại tin này không?',//'Bạn phải mất 1000 VNĐ cho lần khôi phục này. Bạn muốn khôi phục sản phẩm này không?',
			Id : Id,
			callback: function() 
			{
				var boxy = Boxy.get(this);
				...
				$.ajax({
					...
					{                        
						if(data.status == 1)
						{
							...
							{
								case 1:
									window.location = baseurl+ "/product/edit-product/id/"+Id; break;
								case 2:
									window.location = baseurl+ "/product/edit-list-product/id/"+Id; break;
								case 3:
									window.location = baseurl+ "/product/edit-product-library/id/"+Id; break;
							}
						}
						...
						else
						{
							$(this).myBoxy (Boxy,{
								type:'error',
								message: listPr.message.msg_error
							});
						}
					}
				});
