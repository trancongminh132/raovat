...
var post = 
{
	...
	showdetail:false,
	indexImage:0,
	fb_share : function(a,b,c,e){
		FB.ui({method:"feed",link:a,name:b,description:c,picture:e});
	},
	callPopupSharelink : function(provider)
	{
		...
		imgToShare = "http://static-v2.123mua.vn/raovat/default/images/logo.png";
		descToShare = $('.row-info .title .span-title').html();
		...
		switch(provider)
		{
			...
			case 'facebook':
				...
				post.fb_share(linkToShare, titleToShare, descToShare, imgToShare);
				break;
			case 'google':
					...
					{        
						...
						if(data.error == 0)
						{
							$(this).myBoxy (Boxy,{
								type:'success',
								message: data.msg,
								afterHide:function(){window.location = data.url;}
							});
						}
						else 
						{
	...
	deletePost : function(obj, Id)
	{
		$(this).myBoxy (Boxy,{
			type: 'confirm',
			message: 'Việc xóa tin đăng sẽ xóa luôn các dịch vụ đang có của tin đăng này. Bạn chắc chắn muốn xóa?',
			Id : Id,
			callback: function() 
			{
				var boxy = Boxy.get(this);			
					...
					{              
						...
						if(data==1)
						{
							$(this).myBoxy (Boxy,{
								...
								afterHide:function(){window.location = Settings.baseurl+"/usercp/danh-sach-tin.html";}
							});                               
						}
						else if(data == "-1")
						{
							$(this).myBoxy (Boxy,{
								type:'alert',
								message: 'Tin rao đang sử dụng dịch vụ. Bạn không thể xóa tin này!'
							});
						}
						else if(data == "-2")
						{
	...
	removeSavePost : function(obj, Id)
	{
		$(this).myBoxy (Boxy,{
			type: 'confirm',
			message: 'Bạn chắc chắn muốn loraovarại tin này khỏi danh sách tin đã lưu?',
			Id : Id,
			callback: function() 
			{
				var boxy = Boxy.get(this);
	...
	showlargeImg:function(obj){
		var src = $(".img_thumb", obj).attr('rel');
		$("#pic-gallerySlider li img").attr('src', src);
		$('.image-nav-item').removeClass('current');
		$(obj).addClass("current");
		this.indexImage = $(obj).index();
	},
	changethumbImg:function(obj){
		var count = $("#img-gallerySlider li").size();
		this.indexImage++;
		if(this.indexImage >= (count-1))
			this.indexImage = -1;
		var src = $("#img-gallerySlider li img").eq(this.indexImage).attr("rel");
		$("#pic-gallerySlider li img").attr("src", src);
		$('.image-nav-item').removeClass('current');
		$("#img-gallerySlider li").eq(this.indexImage).addClass("current");
	}
};
$(function()
{
	$('.image-nav-item').eq(0).addClass('current');
	//delete post
	...
	$('.up_post').live('click', function ()
	{
		...
		var post_id = arrayVal[0];
		var user_id = arrayVal[0];
		var msg = "";
		if(Account.UID != user_id)
			msg = "Tin này không phải do bạn đăng. Bạn có chắc chắn muốn UP nhanh cho tin đăng này? <br/><span style='padding-left:39px;color:red'> (Chúng tôi không hoàn lại khi bạn sử dụng tính năng này.)</span>"
		else
			msg = "Bạn chắc chắn muốn UP nhanh cho tin đăng này";
		$(this).myBoxy (Boxy,{
			type: 'confirm',
			message: msg,
			callback: function() 
