...
var verSlider = {
    ...
    init: function (e) {
		...
		verSlider.totalItem = e.length;
        this.run()
    },
    run: function () {
        clearTimeout(verSlider.process_run);	
        if (verSlider.queue.length >= 1 && verSlider.totalItem > 10) 
		{
            var e = verSlider.queue.pop();
			if(typeof e.title != "undefined")
				$("#vip_post_123mua").prepend(templateleftpost("post_vip", e.title, e.url, e.post_image, e.price, e.star));
            var t = verSlider.element.find("li.sticky");
            verSlider.element.css("top", -1 * 55);
