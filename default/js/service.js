...
{	
	...
	$('.end_date').live('change', function(){
		...
		{
			if(service.numberFree != 0)
			{
				if(service.numberFree > parseInt(service.type_post.length)*numberDate)
				{
					$('.price_type_post').html("0 ");
				}else
				{
					$('.price_type_post').html((parseInt(service.type_post.length)*numberDate - service.numberFree)+" ");
				}
			}else{
				$('.price_type_post').html((parseInt(service.type_post.length)*numberDate)+" ");
			}
		}
		if($('.price_type_zone').html() != null)
		{
			$('.price_type_zone').html((parseInt(service.type_zone.length)*numberDate)+" ");
		}
