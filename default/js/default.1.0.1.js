var pagedefault = 
{
    ...
    handleScroll:function()
	{
		if (this.processScroll)
		{
			...
			if(parseInt(this.offsetTop) % 100 >= 0 && parseInt(this.offsetTop) % 100 < 10)
			{
				...
				$(".tips a").simpletip({ fixed: true,hideEffect: "none",showEffect: "none"});
	...
	buildULtempalate:function(posts)
	{
		...
		{
			var post = posts[i];
			html.push('<li><div class="line tips"><span class="pic ');
			if(post.post_image == "" || post.post_image == null)
				html.push("pic-default");
			html.push('"');
			html.push('><img alt="" src="'+post.post_image+'"> </span><a target="_blank" rel="'+post.summary+'" href="'+post.url+'">'+post.title);
			if(typeof post.hot != "undefined" && post.hot == 1)
				html.push('<img src="'+Settings.imgurl+'/icon_hot.gif" alt="">');
			if(typeof post.new != "undefined" && post.new == 1)
				html.push('<img src="'+Settings.imgurl+'/icon_new.gif" alt="" style="padding-left:5px">');
			html.push('</a></div></li>');
		}
		return html.join("");
	},
	...
	{
		var html = [];
			html.push('<li class="category-box"><span title="'+category.category_name+'" tracking="homepage_block_category_'+category.category_alias+'" tracking_category="homepage" class="title _trackLink"><span class="icon-'+category.category_icon+'"></span>'+category.category_name+'</span>');
