				...
				$.ajax({
					...
					{              
						...
						else if(data == "-2")
						{
							$(this).myBoxy (Boxy,{
								type:'alert',
								message: 'Tin đang được đăng ký UP tự động bởi hệ thống. Vui lòng hủy đăng ký ở mục "UP tự động" trước khi xóa tin này'
							});
						}else if(data == "-3")
						{
							$(this).myBoxy (Boxy,{
								type:'alert',
								message: 'Bạn không thể xóa tin của người khác đăng!'
							});
						}
						else
						{
				...
				$.ajax({
					...
					{          
						...
						if(data.status == 1)
						{
							$(this).myBoxy (Boxy,{
								type:'success',
								message: 'Bạn đã khôi phục tin thành công!',
...
$(function()
{
	...
	var validator = $("#frmPost").validate({
		...
		messages: {
			...
			description:{required:"Bạn chưa nhập nội dung của tin đăng"},
	...
	$("#submit").click(function(e)
	{
		...
		{
			$("#description").val(post.stripTags($('#post_content').html()));	
			$('#frmPost').submit(); 
		}
		else
		{
	...
	$("#raovat_not_price").click(function(){
		if($(this).attr("checked")){
			$('.select-price').prepend('<div class="layer-hide"></div>');
