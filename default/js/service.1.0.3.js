var service  = 
{
	...
	doReg:function()
	{
		...
		$.ajax({
			...
			data:{id:service.ID, type:service.type_service, number_date:service.numDate,start_date:startDate,end_date:endDate,icon:service.type_icon, price_total:priceTotal},
			success: function(data)
			{
				if(data == 1)
				{
				...
				}else
				{
					...
					new Boxy(data, {modal: true, unloadOnHide: true, afterShow: function(){
						$('.popupConfirm .btn-accept').live('click', function()
						{
							$.ajax({
								type: "POST",
								url: Settings.baseurl + '/service/do-register-service',
